...
with open("services/judge_svc.py", "r") as f:
    content = f.read()

os.makedirs("services/judge", exist_ok=True)

# 1. gold.py
gold_code = """from typing import Optional
...
"""
with open("services/judge/gold.py", "w") as f:
    f.write(gold_code)

# 2. judge.py
judge_code = """import re
import time
import traceback
...
"""
with open("services/judge/judge.py", "w") as f:
    f.write(judge_code)

# 3. __init__.py
init_code = """from .gold import get_gold_key, get_gold_answers, load_raw_json
from .judge import get_llm_server_url, parse_judge_json, query_judge_model, judge_benchmark
"""
with open("services/judge/__init__.py", "w") as f:
    f.write(init_code)

# 4. judge_svc.py (shim)
shim_code = """from services.judge import (
    get_gold_key, get_gold_answers, load_raw_json,
    get_llm_server_url, parse_judge_json, query_judge_model, judge_benchmark
)
from utils.common import get_quantization_from_name # noqa: F401
"""
with open("services/judge_svc.py", "w") as f:
    f.write(shim_code)
