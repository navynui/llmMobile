...
### 4. 📊 Automated AI-as-a-Judge Benchmarking & Multi-Run Aggregation
The system features an automated, background-scheduled 5-round evaluation suite with multi-run statistical post-processing:
...
* **High-Efficiency Execution Modes:** Choose between **`full`** (all 5 rounds, ~15 mins) and **`fast_screen`** (3 core rounds: Knowledge QA + Code Gen + Abstract Reasoning, ~3–4 mins) with dynamic VRAM cooldowns (5s for models under 11 GB).
* **Multi-Run Score Aggregation:** Automatically preserves up to 5 historical runs per model/server, calculating aggregated Mean Score ($\mu$), Standard Deviation ($\sigma$), score variance, and run count ($N$).
* **Score Error Bars in Bubble Chart:** The VRAM-vs-Score bubble chart now draws a white **±σ standard-deviation whisker** (with end caps) through the center of every bubble whose model has multi-run variance (`score_stddev > 0`). Single-run models are left untouched, so score stability is readable at a glance.
* **Automated Model Categorization:** Auto-classifies models into 5 target categories: `⚡ Speed-First` ($\ge 60\text{ TPS}$), `🧠 Reasoning` (avg scores $\ge 14/18$), `🔋 VRAM-Efficient` ($<12\text{GB}$ VRAM), `⚖️ Balanced`, and `🎯 Specialized`. Filter rankings instantly by category pills.
...
* **Queue Benchmarking:** Automatically loads, tests, grades, and switches back and forth between test models and the designated Judge model to keep VRAM footprints clean. Supports **multi-run passes**: pick a mode (`full` / `fast_screen`) and a **Run Count**, and the queue runs N full benchmark passes per model — each stored as its own historical run — then grades them all in one batch so you can queue several models overnight and wake up to aggregated `avg Score + stddev`, `N runs` badges, and category pills.
* **Execution Mode UI Selector:** The Benchmarks tab has a **Mode** dropdown (`full` / `fast_screen` / `speed_multi`) and a **Run Count** input controlling both single-run and queue launches. `fast_screen` trims to 3 core rounds (~3–4 min/run). (`speed_multi` and single-path run_count are still pending backend implementation — see `docs/MultiRunPhaseS.md`.)
...
│   │   └── aggregation.py        # Multi-run statistical aggregation & auto-categorization
...
├── scripts/                      # Host-side recovery & benchmark-repair tooling
│   ├── restore_benchmark_runs.py # Re-insert test_runs rows from saved JSON results
│   ├── regrade_restored_runs.py  # Re-grade restored runs with the AI Judge (resumable)
│   └── regrade.sh                # Convenience wrapper (fixed judge model + localhost:8080)
...
| Endpoint | Method | Description |
|---|---|---|
| `/api/benchmarks` | `GET` | Queries SQLite. Returns ranked list with **3 Quality Filters**, `avg_score`, `score_stddev`, `category_label`, and `runs_count` |
| `/api/benchmarks/run` | `POST` | Starts a background benchmark — supports `execution_mode` (`full`/`fast_screen`) and `temperature` params |
| `/api/benchmarks/aggregate` | `POST` | Manually triggers multi-run statistical aggregation & auto-categorization for a model |
...
* **Run Retention Policy:** Benchmark re-runs use `prune_old_runs(model_id, max_keep=5)` to keep the last 5 historical runs per model (never hard-delete all). `ON DELETE CASCADE` on `round_scores` and `model_hallucinations` handles orphan cleanup automatically. Statistical aggregates (`avg_total_score`, `score_stddev`, `category`) are recalculated after every grading via `calculate_and_store_model_aggregates(model_id)`.
...
- **Phase S – Multi-Run Score Averaging, High-Efficiency Execution & Auto-Categorization**: Implemented a complete multi-run benchmarking and statistical analysis system:
  - **DB Schema Extensions** (`utils/db_utils.py`): Added `run_number`, `run_group_id`, `execution_mode`, `temperature` to `test_runs`; added `category`, `avg_total_score`, `avg_tps`, `score_stddev`, `runs_count` to `models`. Non-destructive run retention policy via `prune_old_runs(model_id, max_keep=5)` replaces the old hard-delete pattern.
  - **High-Efficiency Execution Modes** (`services/benchmark/runner.py`): `fast_screen` mode runs only 3 core rounds (Knowledge QA + Code Gen + Abstract Reasoning) in ~3–4 mins; `full` mode runs all 5 rounds. Dynamic VRAM cooldown: 5s for models $< 11\text{GB}$, 10s for larger models.
  - **Statistical Aggregation Engine** (`services/benchmark/aggregation.py`): New module calculates mean score ($\mu$), standard deviation ($\sigma$), average TPS, and run count across all retained runs. Auto-triggered by the AI Judge after each grading round. Also available via `POST /api/benchmarks/aggregate`.
  - **Auto-Categorization** (`classify_model()`): Models are automatically classified into `⚡ Speed-First`, `🧠 Reasoning`, `🔋 VRAM-Efficient`, `⚖️ Balanced`, `🎯 Specialized`, or `❓ Unclassified` based on aggregated TPS, reasoning/code scores, and VRAM footprint.
  - **Frontend UI**: Benchmark table displays `★ μ ± σ` score chips and `🔄 N runs` badges. Category filter pills in the toolbar. Model details modal includes a statistical summary card and a full historical run-history list.
  - **Multi-Pass Queue (follow-up)**: The queue (`run_benchmark_queue_task`) accepts `execution_mode`, `run_count`, and `temperature`; it runs N `full`/`fast_screen` passes per model (each with its own `test_runs` row + `run_number`/`run_group_id`), then batch-grades all passes with the Judge once. This is the supported flow for overnight multi-run benchmarking (select models ⟶ set Run Count ⟶ **🚀 Run Automated Queue Benchmark**).

### Benchmark Data Recovery & Score Visualization (Phase T)
- **Phase T – Run-Recovery Scripts & Bubble-Chart Error Bars**: Recovered per-model statistics after historical `round_scores` rows were lost:
  - **`scripts/restore_benchmark_runs.py`**: Re-inserts `test_runs` rows (metadata only) for every `benchmark_*.json` still present in `benchmark_results/`, honoring the 5-run retention window (`--max-keep 5`). Dry-run by default — use `--apply` to write. Derives model/server/execution-mode deterministically from existing data.
  - **`scripts/regrade_restored_runs.py`**: Re-runs the AI-Judge pipeline (`judge_benchmark()`) over restored runs so `round_scores` (and hence `avg_total_score`, `score_stddev`, `runs_count`) come back **without re-running the benchmarks** — responses come from the saved JSON. Resumable (skips runs that already have scores), `--redo` re-grades runs whose rounds recorded a grading failure, `--only-model`/`--limit` for smoke tests.
  - **Host-safe judge calls**: the wrapper passes the **exact resolved preset id** (e.g. `NVIDIA-…Q5_K_M.gguf`) into the judge pipeline instead of the lowercased user-facing id, so `localhost:8080` accepts every request (the in-pipeline default container hostname `llm-server` is not reachable from the host).
  - **Bubble-chart error bars** (`src/components/benchmark-bubble-chart.js`, Chart.js): white `±score_stddev` whiskers with end caps drawn only when `score_stddev > 0`; tooltips show `± StdDev (N runs)`.
  - **`consolidate_database()` hardening** (`utils/db_utils.py`): duplicate-model merging now re-assigns **all** runs (including `.gguf` variants) to the clean id before applying the 5-run retention window, preserves aggregate fields (`category`, `avg_*`, `score_stddev`, `runs_count`), and merges hallucinations across variants — no hard deletes of surviving history.
