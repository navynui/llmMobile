...
export class ServerTab extends LitElement {
  ...
  constructor() {
    ...
    this.status = {
      ...
      servers: [
        ...
        {
          name: 'llama-server-mini',
          container: 'llm-server-mini',
          label: 'Secondary (llama-server-mini)',
          status: 'stopped',
          image: null,
          uptime: null,
        },
  ...
  async handleServerAction(serverName, action) {
    return logic.handleServerAction(this, serverName, action);
  }
  ...
  updated(changedProperties) {
    if (changedProperties.has('activeModel')) {
      ...
      if (newActive && newActive !== oldActive) {
        this.dispatchEvent(
          new CustomEvent('op-queue-notification', {
